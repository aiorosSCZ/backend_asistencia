...
try:
    if not firebase_admin._apps:
        if os.path.exists(SERVICE_ACCOUNT_PATH):
            ...
            print("[SUCCESS] Firebase Admin SDK inicializado correctamente.")
        else:
            print("[WARNING] ADVERTENCIA: No se encontró 'serviceAccountKey.json'. Modo Simulación activo.")
...
def send_push_notification(fcm_token: str, titulo: str, mensaje: str, data: dict = None):
    ...
    try:
        ...
        print(f"[SUCCESS] Notificación Push enviada con éxito. ID: {response}")
        return True
    except Exception as e:
        print(f"[ERROR] Error enviando Push: {e}")
        return False
